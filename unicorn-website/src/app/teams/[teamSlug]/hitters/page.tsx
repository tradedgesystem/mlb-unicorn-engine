...
import { getTeamIdFromSlug, slugifyPlayer, slugifyTeam } from "../../../../lib/slugs";

type Team = { team_id: number; team_name: string; abbrev: string };

export default function TeamHittersPage({ params }: { params: { teamSlug: string } }) {
  ...
  return (
    <main className="min-h-screen px-4 py-8 sm:px-8 lg:px-16 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm text-neutral-500">Hitters</p>
      ...
      <div className="glass rounded-3xl p-6 space-y-2">
        {team?.hitters?.length ? (
          ...
          <p className="text-neutral-600">No hitters listed.</p>
        )}
      </div>
    </main>
  );
