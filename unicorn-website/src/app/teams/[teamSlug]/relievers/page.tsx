...
export default function TeamRelieversPage({ params }: { params: { teamSlug: string } }) {
  ...
  return (
    <main className="min-h-screen px-4 py-8 sm:px-8 lg:px-16 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm text-neutral-500">Relievers</p>
      ...
      <div className="glass rounded-3xl p-6 space-y-2">
        {team?.relievers?.length ? (
          <ul className="divide-y divide-white/40">
            {team.relievers.map((p: any) => (
          ...
          <p className="text-neutral-600">No relievers listed.</p>
        )}
      </div>
    </main>
  );
